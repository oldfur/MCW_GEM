...
class EquiTransVariationalDiffusion(torch.nn.Module):
    ...
    def sample_p_zs_given_zt(self, s, t, zt, zt_l, zt_a, 
        ...
        alpha_s_length = self.alpha(gamma_s_length, target_tensor=zt_l)
        alpha_t_length = self.alpha(gamma_t_length, target_tensor=zt_l)
        alpha_s_angle = self.alpha(gamma_s_angle, target_tensor=zt_a)
        alpha_t_angle = self.alpha(gamma_t_angle, target_tensor=zt_a)
    ...
    def reshape_outputs(self, outs, B, N, node_mask):
        """
        outs: 模型输出字典
        B, N: batch_size 和最大原子数
        node_mask: [B, N, 1]
        """
        ...
        pred_atom_types = self.reshape_atom_types(atom_types_flat, node_mask, self.num_classes)

        return pred_x, pred_atom_types, pred_lengths, pred_angles


    def reshape_atom_types(self, atom_types_flat, node_mask, num_classes_charges):
        """
        atom_types_flat: [N_total, H], 模型输出 (softmax 后)
        node_mask: [B, N, 1], 有效原子掩码
        num_classes_charges: H
        return: [B, N, H]
        """
        B, N, _ = node_mask.shape
        device = atom_types_flat.device
        H = num_classes_charges

        # 初始化 [B,N,H]
        atom_types_full = torch.zeros(B, N, H, device=device)

        # 展平 mask -> [N_total]
        mask_flat = node_mask.view(-1).bool()

        # 将输出填回去
        atom_types_full.view(B*N, H)[mask_flat] = atom_types_flat
