...
class EnVariationalDiffusion_pure_x(torch.nn.Module):
    """
    ...
    def __init__(
        ...
        print("init EnVariationalDiffusion_pure_x...")
    ...
    def evaluate_property_mp20(self, x, h, org_context, node_mask=None, edge_mask=None):
        # Normalize data, take into account volume change in x.
        batch_size = x.size(0)
        x, h, delta_log_px = self.normalize(x, h, node_mask)
    ...
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, 
        ...
        if self.property_pred:
            if self.atom_type_pred:
                z[:,:,self.n_dims:] = 1    
            x, h, pred = self.sample_p_xh_given_z0(
