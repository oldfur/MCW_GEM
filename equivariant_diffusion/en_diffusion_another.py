...
class EnVariationalDiffusion_another(torch.nn.Module):
    """
    ...
    def __init__(
        ...
        print("EnVariationalDiffusion another...")
    ...
    def kl_prior_mp20(self, xh, node_mask):
        """
    ...
    def compute_loss(self, x, h, lengths, angles, node_mask, edge_mask, context, t0_always, mask_indicator=None,
        ...
        # phi为每个时间步的去噪神经网络
        if self.property_pred:
            (net_out, property_pred), lengths_pred, angles_pred = self.phi(z_t, t, node_mask, edge_mask, context)
        else:
            # Neural net prediction.
            net_out, lengths_pred, angles_pred = self.phi(z_t, t, node_mask, edge_mask, context)
        ...
        # The KL between q(zT | x) and p(zT) = Normal(0, 1). Should be close to zero.
        kl_prior = self.kl_prior_mp20(xh, node_mask)
        ...
        if t0_always:   # test
            ...
            if self.property_pred:
                (net_out, property_pred), lengths_pred, angles_pred = self.phi(z_0, t_zeros, node_mask, edge_mask, context)
            else:
                net_out, lengths_pred, angles_pred = self.phi(z_0, t_zeros, node_mask, edge_mask, context)
            ...
            # and this will later be selected via masking.
            loss_term_0 = -self.log_pxh_given_z0_without_constants(
                x, h, z_t, gamma_t, eps, net_out, node_mask)   # main loss term
