...
class EnVariationalDiffusion_concat(torch.nn.Module):
    """
    The E(n) Diffusion Module.
    """
    def __init__(
            self,
            dynamics: EGNN_dynamics_MP20, in_node_nf: int, n_dims: int,
        ...
        print("EnVariationalDiffusion concat version...")
    ...
    def kl_prior_mp20(self, xhla, node_mask):
        """
        计算 q(zT | x) 与 p(zT) = N(0, 1) 的KL散度
        """
        # x, h部分
        ones = torch.ones((xhla.size(0), 1), device=xhla.device)
        gamma_T = self.gamma(ones)
        alpha_T = self.alpha(gamma_T, xhla)
        mu_T = alpha_T * xhla
        mu_T_x, mu_T_hla = mu_T[:, :, :self.n_dims], mu_T[:, :, self.n_dims:]
        sigma_T_x = self.sigma(gamma_T, mu_T_x).squeeze()
        sigma_T_hla = self.sigma(gamma_T, mu_T_hla)
        zeros, ones_tensor = torch.zeros_like(mu_T_hla), torch.ones_like(sigma_T_hla)
        kl_distance_hla = gaussian_KL(mu_T_hla, sigma_T_hla, zeros, ones_tensor, node_mask)
        ...
        return kl_distance_x + kl_distance_hla
    ...
    def sample_p_xh_given_z0(self, z0, node_mask, edge_mask, context, fix_noise=False):
        ...
        if self.atom_type_pred:
            xh = self.sample_normal(mu=mu_x[:,:,:3], sigma=sigma_x, node_mask=node_mask, fix_noise=fix_noise, only_coord=True)
        else:
            xh = self.sample_normal(mu=mu_x, sigma=sigma_x, node_mask=node_mask, fix_noise=fix_noise)
        x = xh[:, :, :self.n_dims]

        h_int = z0[:, :, self.n_dims+self.num_classes:self.n_dims+self.num_classes+1] \
            if self.include_charges else torch.zeros(0).to(z0.device)
        if self.dynamics.mode == "PAT" or self.atom_type_pred:
    ...
    def sample_p_xh_lengths_angles_given_z0(self, z0, node_mask, edge_mask, context, fix_noise=False):
        ...
        if self.atom_type_pred:
            xhla = self.sample_normal(mu=mu_x[:,:,:3], sigma=sigma_x, node_mask=node_mask, fix_noise=fix_noise, only_coord=True)
        else:
            xhla = self.sample_normal(mu=mu_x, sigma=sigma_x, node_mask=node_mask, fix_noise=fix_noise)
        x = xhla[:, :, :self.n_dims]
        ...
        if self.atom_type_pred:
            ...
            x, h_cat, h_int = self.unnormalize(x, z0[:, :, self.n_dims:self.n_dims+self.num_classes], h_int, node_mask)

        h_cat = F.one_hot(torch.argmax(h_cat, dim=2), self.num_classes) * node_mask
        h_int = torch.round(h_int).long() * node_mask
        l_sampled = net_out[:, :, -6:-3]
        a_sampled = net_out[:, :, -3:]
        l_sampled = l_sampled * node_mask
        a_sampled = a_sampled * node_mask

        h = {'integer': h_int, 'categorical': h_cat}
        
        if self.property_pred:
            return x, h, pred, l_sampled, a_sampled
        return x, h, l_sampled, a_sampled
    ...
    def log_pxhla_given_z0_without_constants(
            self, x, h, la, z_t, gamma_0, eps, net_out, node_mask, epsilon=1e-10):
        # Discrete properties are predicted directly from z_t.

        z_h_cat = z_t[:, :, self.n_dims:self.n_dims+self.num_classes]
        z_h_int = z_t[:, :, self.n_dims+self.num_classes:self.n_dims+self.num_classes+1] if self.include_charges \
            else torch.zeros(0).to(z_t.device)
        z_la = z_t[:, :, -6:].mean(dim=1)

        # Take only part over x.
        eps_x = eps[:, :, :self.n_dims]
        net_x = net_out[:, :, :self.n_dims]
        eps_la = eps[:, :, -6:].mean(dim=1)
        net_la = net_out[:, :, -6:].mean(dim=1)
        ...
        # compute l and a part
        log_p_la_given_z_without_constants = -0.5 * sum_except_batch((eps_la - net_la) ** 2)
        ...
        # Select the log_prob of the current category us-
        # ing the onehot representation.
        ...
        log_p_xh_given_z = log_p_x_given_z_without_constants + log_p_h_given_z + log_p_la_given_z_without_constants
    ...
    def compute_loss(self, x, h, lengths, angles, node_mask, edge_mask, context, t0_always, mask_indicator=None,
        ...
        :param x: [B, N, 3]
        :param h: [B, N, num_classes + include_charges]
        ...
        batch_size = x.size(0)
        # Concatenate x, h[integer] and h[categorical], lengths, angles
        n_nodes = x.size(1)
        lengths_extend = lengths.unsqueeze(1).repeat(1, n_nodes, 1)
        angles_extend = angles.unsqueeze(1).repeat(1, n_nodes, 1)
        xhla = torch.cat([x, h['categorical'], h['integer'], lengths_extend, angles_extend], dim=2)
        hla = torch.cat([h['categorical'], h['integer'], lengths_extend, angles_extend], dim=2)
        la = torch.cat([lengths_extend, angles_extend], dim=2)
        fix_h_la = torch.ones_like(torch.cat([h['categorical'], h['integer'], lengths_extend, angles_extend], dim=2))
        ...
        if self.atom_type_pred:
            z_t = alpha_t * x + sigma_t * eps[:, :, :self.n_dims]  
            z_t = torch.cat([z_t, fix_h_la], dim=2)
        else:
            z_t = alpha_t * xhla + sigma_t * eps
        ...
        ### Compute the error.
        error = self.compute_error_mp20(net_out, eps)
        ...
        kl_prior = self.kl_prior_mp20(xhla, node_mask)
        ...
        if t0_always:   # test
            ...
            if self.atom_type_pred:
                z_0 = alpha_0 * x + sigma_0 * eps_0[:, :, :self.n_dims]
                z_0 = torch.cat([z_0, fix_h_la], dim=2)
            else:
                z_0 = alpha_0 * xhla + sigma_0 * eps_0
            ...
            loss_term_0 = -self.log_pxhla_given_z0_without_constants(x, h, la, z_0, gamma_0, eps_0, net_out, node_mask)
            ...
            loss_term_0 = -self.log_pxhla_given_z0_without_constants(
                x, h, la, z_t, gamma_t, eps, net_out, node_mask)   # main loss term
        ...
        """至此loss已经计算完成, 下面mask掉不需要计算的loss, 等同于计算生长阶段loss"""

        # calc loss for lengths, angles
        if node_mask.dim() == 2:  # [B, N]
            node_mask = node_mask.unsqueeze(-1)  # [B, N, 1]
        valid_counts = node_mask.sum(dim=1)  # [B, 1]
        lengths_pred = net_out[:, :, -6:-3]
        angles_pred = net_out[:, :, -3:]
        # 屏蔽无效节点, 在节点维度上求和
        l_masked = lengths_pred * node_mask  # [B, N, 3]
        a_masked = angles_pred * node_mask  # [B, N, 3]
        l_sum = l_masked.sum(dim=1)  # [B, 3]
        a_sum = a_masked.sum(dim=1)  # [B, 3]
        # 求平均, 避免除零
        l_mean = l_sum / (valid_counts + 1e-8)  # [B, 3]
        a_mean = a_sum / (valid_counts + 1e-8)  # [B, 3]
        ...
        if self.dynamics.mode == "PAT" or self.atom_type_pred:
            ...
            h_pred = net_out[:, :, self.n_dims:(self.n_dims+self.num_classes+self.include_charges)].to(x.device)
        ...
        if self.property_pred:
            ...
            pred_loss_mask = (t_int <= self.prediction_threshold_t).float() # [B, 1]
            # 1--nucleation time(x,h,context loss); 0--growth time(x loss); Importmant!
    ...
    def sample_p_zs_given_zt(self, s, t, zt, node_mask, edge_mask, context, 
        ...
        if self.atom_type_pred:
            atom_type_pred = eps_t[:, :, 3:3+self.num_classes]
            # we always include the charges!
            charges_pred = eps_t[:,:, 3+self.num_classes:3+self.num_classes+self.include_charges]
            la_pred = eps_t[:, :, -6:]  # lengths and angles
             # only use the coordinate part to compute mu
        ...
        zs = self.sample_normal(mu, sigma, node_mask, fix_noise, only_coord=True if self.atom_type_pred else False)

        # Project down to avoid numerical runaway of the center of gravity.
        if self.atom_type_pred:
            zs = torch.cat(
                [diffusion_utils.remove_mean_with_mask(zs[:, :, :self.n_dims], node_mask),
                atom_type_pred, charges_pred, la_pred], dim=2
            )
        else:
            zs = torch.cat(
                [diffusion_utils.remove_mean_with_mask(zs[:, :, :self.n_dims], node_mask),
    ...
    def sample_combined_position_feature_noise(self, n_samples, n_nodes, node_mask):
        ...
        z_h = utils.sample_gaussian_with_mask(
            size=(n_samples, n_nodes, self.num_classes + self.include_charges), device=node_mask.device,
            node_mask=node_mask)
        z_la = utils.sample_gaussian_with_mask(
            size=(n_samples, n_nodes, 6), device=node_mask.device,
            node_mask=node_mask)
        z = torch.cat([z_x, z_h, z_la], dim=2)
        return z
    ...
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, 
        ...
        for s in reversed(range(0, self.T)):
            ...
            if self.atom_type_pred:
                z[:, :, self.n_dims:] = 1 
                z = self.sample_p_zs_given_zt(s_array, t_array, z[:,:,:3], node_mask, edge_mask, 
                                              context, fix_noise=fix_noise, pesudo_context=pesudo_context)
            else:
                z = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, 
        ...
        if self.property_pred:
            if self.atom_type_pred:
                z[:,:,self.n_dims:] = 1    
            x, h, pred, l_sampled, a_sampled = self.sample_p_xh_lengths_angles_given_z0(
                z, node_mask, edge_mask, context, fix_noise=fix_noise
                )
        else:
            if self.dynamics.mode == "PAT" or self.atom_type_pred:
                z[:,:,self.n_dims:] = 1 # set the atom type to 1 for PAT
            x, h, l_sampled, a_sampled = self.sample_p_xh_lengths_angles_given_z0(
                z, node_mask, edge_mask, context, fix_noise=fix_noise
            )
        ...
        valid_counts = node_mask.sum(dim=1)  # [B, 1]
        # 屏蔽无效节点, 在节点维度上求和
        l_sampled_masked = l_sampled * node_mask  # [B, N, C]
        a_sampled_masked = a_sampled * node_mask  # [B, N, C]
        l_sampled_sum = l_sampled_masked.sum(dim=1)  # [B, C]
        a_sampled_sum = a_sampled_masked.sum(dim=1)  # [B, C]
        # 求平均 (避免除零)
        l_sampled_mean = l_sampled_sum / (valid_counts + 1e-8)  # [B, C]
        a_sampled_mean = a_sampled_sum / (valid_counts + 1e-8)  # [B, C]

        if self.property_pred:
            return x, h, pred, l_sampled_mean, a_sampled_mean
        return x, h, l_sampled_mean, a_sampled_mean
