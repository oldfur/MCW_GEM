...
from egnn.Equiformer_dynamics_F import EquiformerV2DynamicsF
from equivariant_diffusion.en_diffusion_trans import EquiTransVariationalDiffusion
from equivariant_diffusion.en_diffusion_Lfirst import EquiTransVariationalDiffusion_Lfirst
from equivariant_diffusion.en_diffusion_Lhard import EquiTransVariationalDiffusion_Lhard
...
from equivariant_diffusion.diffusion_L import VariationalDiffusion_L
...
def get_Lattice_model(args, device, dataset_info, uni_diffusion=False, pretrain=False):
    ...
    if args.LatticeGenModel == 'diffusion_L':
        net = VariationalDiffusion_L(
...
def get_model(args, device, dataset_info, dataloader_train, 
    ...
    # print(net_dynamics)
    ...
    elif args.probabilistic_model == 'diffusion_Lfirst' or args.probabilistic_model == 'diffusion_Lhard':
        net_dynamics = EquiformerV2Dynamics(hidden_dim=128, latent_dim=0, 
            max_neighbors=12, radius=12., condition_time='embed', 
            time_dim=128, embed_noisy_types=False, regress_energy=False, 
            regress_forces=True, regress_atoms=True, regress_lattices=True,
    ...
    elif args.probabilistic_model == 'diffusion_LF' or args.probabilistic_model == 'diffusion_LF_wrap':
        net_dynamics = EquiformerV2DynamicsF(hidden_dim=128, latent_dim=0,            
            max_neighbors=12, radius=12., condition_time='embed', 
            time_dim=128, embed_noisy_types=False, regress_energy=False, 
            regress_forces=True, regress_atoms=True, regress_lattices=True,
            embed_lattices=True, embed_coord=False,
        ...
        args.probabilistic_model == 'diffusion_L_another':
        net_dynamics = None
    else:
    ...
    if args.probabilistic_model == 'diffusion':
    ...
    elif args.probabilistic_model == 'diffusion_Lfirst':
        vdm = EquiTransVariationalDiffusion_Lfirst(
            n_dims=3, device=device,
            dynamics=net_dynamics,
            in_node_nf=in_node_nf,
            pre_training=pretrain,
        ...
        vdm = EquiTransVariationalDiffusion_Lhard(
            n_dims=3, device=device,
            dynamics=net_dynamics,
            in_node_nf=in_node_nf,
            pre_training=pretrain,
    ...
    elif args.probabilistic_model == 'diffusion_L_another':
        vdm = VariationalDiffusion_L_another(
            n_dims=3, device=device,
            dynamics=net_dynamics,
            in_node_nf=in_node_nf,
            pre_training=pretrain,
    ...
    elif args.probabilistic_model == 'diffusion_LF':
        vdm = EquiTransVariationalDiffusion_LF(
            n_dims=3, device=device,
            dynamics=net_dynamics,
            in_node_nf=in_node_nf,
            pre_training=pretrain,
        ...
        vdm = EquiTransVariationalDiffusion_LF_wrap(
            ...
            geometry_correction_mode=getattr(args, 'geometry_correction_mode', 'default'),
            zbl_correction_inner_steps=getattr(args, 'zbl_correction_inner_steps', 20),
            zbl_correction_step_size=getattr(args, 'zbl_correction_step_size', 1e-3),
            zbl_correction_r_cut=getattr(args, 'zbl_correction_r_cut', 0.8),
            zbl_correction_force_clip=getattr(args, 'zbl_correction_force_clip', 100.0),
            zbl_correction_max_step=getattr(args, 'zbl_correction_max_step', 0.02),
            interleaved_zbl_steps=getattr(args, 'interleaved_zbl_steps', 10),
            interleaved_zbl_step_size=getattr(args, 'interleaved_zbl_step_size', 1e-3),
            interleaved_zbl_r_cut=getattr(args, 'interleaved_zbl_r_cut', 0.8),
            interleaved_zbl_force_clip=getattr(args, 'interleaved_zbl_force_clip', 100.0),
            interleaved_zbl_max_step=getattr(args, 'interleaved_zbl_max_step', 0.02),
