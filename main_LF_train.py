...
from mp20.train_epoch import train_epoch_F
from mp20.analyze_test import analyze_and_save_F, test_F
...
def main(args):
    ...
    if args.pretrained_Lattice_model:
        ...
        miss_key, unexcept_key  =  LatticeGenModel.load_state_dict(new_state_dict, strict=False)
        print(f"load from {args.pretrained_Lattice_model}, miss_key: {miss_key}")
        print("LatticeGenModel constructed successfully.")
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        train_epoch_F(args=args, dataloader=dataloaders['train'], epoch=epoch, model_dp=model_dp,
        ...
        if epoch % args.test_epochs == 0 and epoch != 0 and epoch >= args.visulaize_epoch:   # 默认每10个epoch测试一次
            ...
            nll_test = test_F(args, dataloaders['test'], dataset_info, epoch, model_ema_dp, 
                        property_norms, nodes_dist, partition='Test')

            # 判断是否需要保存
            need_save = False
            if nll_val < best_nll_val:
                print("best val loss model at epoch %d" % epoch)
                best_nll_val = nll_val
                need_save = True     
            if nll_test < best_nll_test:
                print("best test loss model at epoch %d" % epoch)
