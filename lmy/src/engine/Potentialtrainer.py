...
class PotentialTrainer:
    def __init__(self, 
        ...
        if kwargs.get("only_les", False):
            for name, param in self.model.named_parameters():
                if "long_range" in name or "sigma" in name:
        ...
        if self.finetune_mode:
            # ---> 分支 A: 长程静电微调模式 (Differential LR) <---
            if self.rank == 0:
                print(f"🔧 [Trainer] Initializing in FINETUNE mode (GNN={lr_gnn}, LES={lr_les})")
            ...
            for name, param in self.model.named_parameters():
                ...
                if "long_range" in name or "sigma" in name:
                    ...
                    les_names.append(name) # 🔥 记录名字
                else:
                    gnn_params.append(param)
                    gnn_names.append(name) # 🔥 记录名字
            ...
            if self.rank == 0:
                ...
                with open(group_log_path, 'w') as f:
                    f.write(f"=== Total Trainable Params: {len(gnn_names) + len(les_names)} ===\n\n")
                    
                    f.write(f"--- Group LES (High LR: {lr_les}) [Count: {len(les_names)}] ---\n")
                    if len(les_names) == 0:
                        f.write("⚠️ WARNING: NO PARAMETERS FOUND IN LES GROUP!\n")
                    for n in les_names:
                        f.write(f"{n}\n")
                    if len(gnn_names) > 0:
                        f.write(f"\n--- Group GNN (Low LR: {lr_gnn}) [Count: {len(gnn_names)}] ---\n")
                        for n in gnn_names:
                            f.write(f"{n}\n")
            ...
            if len(gnn_names) > 0:
                self.optimizer = optim.AdamW([
                    {'params': gnn_params, 'lr': lr_gnn, 'weight_decay': 1e-2}, # GNN 慢学
                    {'params': les_params, 'lr': lr_les, 'weight_decay': 0}   # LES 快学
                ], amsgrad=True)
            else:
                self.optimizer = optim.AdamW([
                    {'params': les_params, 'lr': lr_les, 'weight_decay': 1e-3}   # LES 快学
                ], amsgrad=True)
            
        else:
            # ---> 分支 B: 原始从头训练模式 (Original Logic) <---
            self.optimizer = optim.AdamW(
                model.parameters(), 
                lr=max_lr, 
                weight_decay=1e-4, 
    ...
    def train_epoch(self, loader, epoch_idx):
        ...
        # 如果 Config 里没写，且是微调模式，我们可以给个默认值 (比如 2000 步)
        ...
        for i, batch in enumerate(pbar):
            ...
            if self.finetune_mode:
                if len(self.optimizer.param_groups) > 1:
                    log_data.update({'epoch': epoch_idx, 'step': i, 'lr': self.optimizer.param_groups[1]['lr']})
                else:
            ...
            if not self.finetune_mode:
                # ---> 分支 B: OneCycleLR 每一步都更新
                self.scheduler.step()
            else:
                # ---> 分支 A: Plateau 调度器在此处不更新 (它在 validate 后更新)
                pass
            ...
            if max_steps is not None and (i + 1) >= max_steps:
                ...
                break # <--- 强制跳出循环，去执行外层的 validate 和 scheduler.step
