...
def main():
    ...
    # 2. 准备文件
    file_dirs = [r"005", r"outcar_selected_xyz", r"xyz_grouped"]
    all_files = []
    for d in file_dirs:
        if os.path.exists(d):
            all_files.extend([os.path.join(d, f) for f in os.listdir(d) if f.endswith('.xyz')])
            
    random.shuffle(all_files)
    
    # 🔥🔥🔥 核心参数建议 🔥🔥🔥
    # 即使你有 60 核，也不要超过 16。IO 瓶颈下，核多反而慢。
    NUM_WORKERS = 32      # 建议 8-12，绝对不要 60
    TRAIN_CHUNK_SIZE = 5120
    SAVE_DIR = "processed_dataset"
    CUTOFF = 6.0
    
    # 划分
    train_end = min(20000, len(all_files))
    test_end = min(21000, len(all_files))
