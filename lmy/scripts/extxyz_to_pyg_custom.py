...
def extxyz_to_pyg_custom(xyz_file_path, cutoff=6.0, require_stress=False):
    ...
    for atoms in frames_iter:
        try:
            ...
            z = torch.from_numpy(atoms.get_atomic_numbers()).to(torch.int8)

            # 2. 坐标: float32 (为了精度，坐标通常建议保留 float64)
            pos = torch.from_numpy(atoms.get_positions()).to(torch.float32)
            
            # 3. 晶胞: float32 -> (1, 3, 3)
            # 注意: array 是 numpy 数组，使用 from_numpy 避免不必要的复制
            cell = torch.from_numpy(atoms.get_cell().array).to(torch.float32).unsqueeze(0)

            # --- B. 标签 (Energy, Force, Stress) ---
            
            # 能量: float32 -> (1,)
            energy_val = atoms.info.get('REF_energy', 0.0)
            y = torch.tensor([energy_val], dtype=torch.float32)
            ...
            if forces_np is None:
                ...
                forces = torch.from_numpy(forces_np).to(torch.float32)
            ...
            if stress_info is not None:
                ...
                stress_tensor = torch.from_numpy(s_mat).to(torch.float32).unsqueeze(0)
            ...
            edge_index = torch.tensor(np.vstack((i_idx, j_idx)), dtype=torch.int32)
            
            # [关键压缩 2] shifts_int: int16 (比 int8 更安全)
            # S_integers 虽然通常很小，但极端情况下可能超过 127，int16 更稳妥
            shifts_int = torch.from_numpy(S_integers).to(torch.int32) # 这里用int32兼容性最好，int16也可以
